...
/**
 * PaymentGatewayProvider - Creates PaymentGateway adapters
 * 
    ...
    public static PaymentGateway createGateway(GatewayType type, String... credentials) {
        switch (type) {
            case STRIPE:
                ...
                return new StripePaymentClient(credentials[0]);

            case PAYPAL:
                if (credentials.length < 2) {
                    throw new IllegalArgumentException("PayPal requires clientId and clientSecret");
                }
                return new PayPalPaymentClient(credentials[0], credentials[1]);

            case SQUARE:
                if (credentials.length < 2) {
                    throw new IllegalArgumentException("Square requires accessToken and locationId");
                }
                return new SquarePaymentClient(credentials[0], credentials[1]);
